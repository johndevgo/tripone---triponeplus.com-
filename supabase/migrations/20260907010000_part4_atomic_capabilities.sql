-- Keep multi-capability onboarding inside the existing idempotent generation transaction.
...
as $$
declare
  uid uuid := auth.uid(); business_id uuid; new_site_id uuid;
  page jsonb; experience jsonb; selected_capabilities text[];
begin
  ...
  select coalesce(array_agg(value), array[payload->>'businessType']) into selected_capabilities
  from jsonb_array_elements_text(coalesce(payload->'capabilities', jsonb_build_array(payload->>'businessType'))) values_list(value);
  perform public.set_business_capabilities(business_id, selected_capabilities, payload->>'businessType');
  ...
  values(business_id,uid,payload->>'name',payload->>'slug',payload->>'themeId',payload->'generated'->'theme',payload->'generated'->'globalSettings',payload->'generated'->'navigation',payload->'generated'->'footer')
  ...
  values(new_site_id,1,payload->'generated',uid,'Initial website');
  return new_site_id;
end
$$;
