-- TripOne+ Part 4 milestone 5: make first-party enquiries and analytics work
-- on the current /s/{siteSlug} delivery route. The route handler derives the
-- delivery mode; PostgreSQL still verifies the requested site is published.
...
begin
  ...
  if payload->>'deliveryMode' = 'fallback' then
    if clean_slug !~ '^[a-z0-9]+(?:-[a-z0-9]+)*$'
      or clean_path not like ('/s/' || clean_slug || '%') then
      raise exception 'Invalid fallback website';
    end if;
...
begin
  ...
  if payload->>'deliveryMode' = 'fallback' then
    ...
    select s.id into target_site from public.sites s
    where s.id = requested_site and s.slug = clean_slug
      and s.status = 'published' and s.published_snapshot is not null
    limit 1;
  else
    select s.id into target_site
